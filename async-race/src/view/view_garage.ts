...
export const drawGarageCars = (name: string, color: string, id: number) => {
  ...
  BUTTON_START.addEventListener('click', (event) => startRace(event?.target));
  BUTTON_STOP.addEventListener('click', (event) => stopRace(event?.target));
