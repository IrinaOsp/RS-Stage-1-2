...
export const drawGarageInputs: () => void = () => {
  ...
  createElem(CONTROL_BLOCK, HTMLTags.input, 'input-name create-name');
...
export const drawGarageCars: (name: string, color: string, id: number) => void = (name, color, id) => {
  ...
  createElem(CONTAINER_HEAD, HTMLTags.span, 'car-name', name);
  ...
  createElem(CAR_CONTAINER, HTMLTags.div, 'flag');
};
