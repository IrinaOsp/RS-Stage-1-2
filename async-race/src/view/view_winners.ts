import { HTMLTags, getWinnersResult } from '../types/types';
...
function setPaginationBtns(page: number, pages: number): void {
  ...
  if (WIN_PREV instanceof HTMLButtonElement && WIN_NEXT instanceof HTMLButtonElement) {
    ...
    WIN_PREV.addEventListener('click', (event) => paginationWinners(event.target, page));
    WIN_NEXT.addEventListener('click', (event) => paginationWinners(event.target, page));
    if (page === 1 && pages === 1) {
...
export function setHeadings(page: number, winnersPages: number, winnersNumber?: number): void {
  if (winnersNumber) {
    const GARAGE_HEADING = document.querySelector('.garage-heading');
    if (GARAGE_HEADING) GARAGE_HEADING.textContent = `Winners (${winnersNumber})`;
  }
  const PAGE_HEADING = document.querySelector('.page-count');
  if (PAGE_HEADING) PAGE_HEADING.textContent = `${page} / ${winnersPages}`;
}

export const drawWinnersLines: (
  page: number,
  sort?: 'id' | 'wins' | 'time',
  order?: 'ASC' | 'DESC',
) => Promise<getWinnersResult> = async (page, sort = 'time', order = 'ASC') => {
  return getWinners([
...
function sortWinners(target: EventTarget | null): void {
  const TARGET = target;
  let order: 'ASC' | 'DESC';
  let sort: 'id' | 'wins' | 'time';
  if (TARGET instanceof HTMLElement) {
    ...
    if (arr) {
      ...
      drawWinnersLines(page[0], sort, order);
    } else {
      drawWinnersLines(1, sort, order);
    }
  }
}

function addSortingListeners(): void {
...
export const drawWinners: (p?: number) => void = (page = currentWinnersPage) => {
  ...
  if (PAGE_ARR) PAGE = JSON.parse(PAGE_ARR).at(0);
  ...
  HEADING_TEXT.forEach((colName, ind) => {
    const TH = createElem(TR, HTMLTags.th, 'winners_thead_tr', colName);
    if (ind === 3) TH.classList.add('th_wins');
    if (ind === 4) TH.className = 'winners_thead_tr th_time ASC';
  });
  createElem(TABLE, HTMLTags.tbody, 'winners_tbody');
  drawWinnersLines(PAGE).then((response) => {
